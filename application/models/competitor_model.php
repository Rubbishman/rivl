...
class Competitor_model extends CI_Model {
	...
	public function get_competitor($competition_id,$id = FALSE)
	{
		...
		$this->db->join('score s2', 's1.game_id = s2.game_id and s1.competitor_id != s2.competitor_id');
		$this->db->join('game', 's1.game_id = game.id and game.competition_id = '.$competition_id);
